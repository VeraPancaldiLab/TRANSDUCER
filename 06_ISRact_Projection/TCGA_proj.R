...
projection_TCGA <-  predict(pca_pdx, human_data) %>% 
  as_tibble(rownames = "sample")
...
show_projection <- bind_rows(as_tibble(pca_full_df) %>% mutate(dataset = "Sauyeun PDX"),
                             as_tibble(projection_TCGA) %>% mutate(ISRact = "Unknown",
                                                                   dataset = "TCGA"))
...
dplyr::mutate(show_projection, ISRact = str_replace(ISRact, 'ICA3', 'ISRact')) %>%
  ...
  geom_rect(xmin = quantile(projection_TCGA$PC1, probs = 0.6666),
            xmax = max(projection_TCGA$PC1),
            ymin = min(projection_TCGA$PC2), ymax = max(projection_TCGA$PC2), linewidth = 0, fill = "green", alpha =0.002)
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# Plot comparisons with Basal/Classical and ISRact
## PAMG vs PC1
correlation_plotter(data = TCGA_PC1, col1 = "PAMG", col2 = "PC1", data_name = "TCGA")
#-------------------------------------------------------------------------------
#-------------------------------------------------------------------------------
# Survival curves regarding ISR status
surv_data <- dplyr::rename(TCGA_PC1, case_id = sample)
## OS
...
ggsave(paste0("results/survplots/FIGURES/kmos_tcga_ISRact.svg"),
       kmos_tcga_ISRact$plot, height = 4 , width = 4)
...
ggsave(paste0("results/survplots/FIGURES/kmpfs_tcga_ISRact.svg"),
       kmpfs_tcga_ISRact$plot, height = 4 , width = 4)
...
ggsave(paste0("results/survplots/FIGURES/kmos_tcga_", measure ,"_phgdhcbs.svg"),
       kmos_tcga$plot, height = 4 , width = 4)
### PFS
