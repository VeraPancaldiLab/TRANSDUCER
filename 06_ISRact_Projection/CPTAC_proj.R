# ISRactPCA projection into CPTAC cohort pt1
################################################################################
#' Import different files
#' Filter data and project ISRactPCA
#' Produce projection scatterplot
#' Compare marker gene transcription
#' sample crosscorrelation plot with Shin et al. PDX
#' Explore relation with PAMG sample score
#' DPEA ISRactPCA high vs low
#' Survival analysis ISRactPCA and PHGDH/CBS
#' Deconvolution proportion correlations
################################################################################
...
if (norm_method == "upperquartile_ogdata") {
  ...
  CPTAC_tumor <- column_to_rownames(CPTAC_tumor_, "Gene") %>%
    DGEList() %>%
...
show_projection <- bind_rows(
  ...
  as_tibble(CPTAC_PCAspace) %>% mutate(
    ISRact_bin = "Unknown",
    dataset = "CPTAC"
  )
)

projection_scatter_cptac <- dplyr::filter(
  show_projection, dataset %in% c("Shin et al. PDX", "PACAOMICS PDX", "CPTAC", "CCLE"), # "Sauyeun PDX","PACAOMICS PDX", "CPTAC", "CCLE"
...
annot_ref <- left_join(sample_info, dplyr::select(top_samples, sample, ISRact_bin)) %>%
  ...
  dplyr::mutate(
    ISRact_bin = if_else(is.na(ISRact_bin), "intermediate_ISRact", ISRact_bin),
    df = "Shin et al. PDX"
  )
...
gw_corr_hm %>%
  column_to_rownames("measure2") %>%
  pheatmap(
    scale = "none", color = colorRampPalette(c("#FBFEF9", "#A63446"))(100),
...
dpea_isractpca_volcano <- ggplot(DEqMS.results, aes(x = logFC, y = log.sca.pval)) +
  ...
  geom_text_repel(
    data = subset(DEqMS.results, abs(logFC) > 0.5 & log.sca.pval > 3),
    aes(logFC, log.sca.pval, label = gene)
  ) + # add gene label
  geom_point(
    data = DEqMS.results[rownames(DEqMS.results) %in% highlight, ],
