...
markers <- FindMarkers(seurat_object, ident.1 = "11", group.by = 'seurat_clusters')
```

```{r}
# Top 10 plot and GSEA
sign_markers <- dplyr::filter(markers, p_val_adj < 0.01)

VlnPlot(seurat_object, features = c(rownames(sign_markers)[1:10]), group.by = "seurat_clusters", pt.size = 0) + geom_hline(yintercept=0, linetype = "dashed")
```
