...
# Hwang AUCell enrichment analysis seeking a ISRact scRNAseq population
...
```
## Exploration of Tumour cell population, clustering and availale metadata
Seeing the lack of any specific population in the general UMAP, how are these cells distributed when only on the Tumor cells are considered? Using all the genes or only ISRact related?
...
if (basalclassical_source == "Raghavan_figures2d"){
  ...
  scclassical <- read.xlsx("data/Other/Raghavan2020_TableS3.xlsx", sheetName =  "S3 scClassical score correlates", startRow = 5) %>%
  ...
  scbasal <- read.xlsx("data/Other/Raghavan2020_TableS3.xlsx", sheetName = "S3 scBasal score correlates", startRow = 5) %>%
    dplyr::slice_head(n=top_genes) %>%
    dplyr::filter(Gene %in% rownames(seurat_object@assays$RNA$data)) %>%
    dplyr::select(Gene) %>%
...
And now I compute the neighbors and cluster this UMAP to compare with ISRact
...
At this moment I compare all these clusters with some violin plots, as well as assert the n of cells of each cluster
...
Now to see what each cluster actually represents we compare each cluster gene expression with the rest of clusters. In this comparsion we will see what actually differentiates these populations.

As a warning, this process takes up to 12h to run, and thus I recommend loading the precomputed data. To carry out the analysis un comment the lines in the following sections

### ISRact low
starting with cluster 10, which seems to be specific of ISRactlow. What are their marker genes?

(At this point to explore a UMAP and clustering with 2 only clusters, independent of the log2countsperecell, that was originated from 
- different UMAP: RunUMAP(seurat_object, reduction = "harmony",dims=c(2:5))
- different resolution: seurat_object <- FindClusters(seurat_object, resolution = 0.01)

you can load "Hwang_AUCell_onlytumor_untilCluster1_alternativeclustering.RDATA" and look at cluster 1)

```{r}
# cluster_to_explore <- "10"
# markers <- FindMarkers(seurat_object, ident.1 = cluster_to_explore, group.by = 'seurat_clusters')
# save.image(file = "Hwang_AUCell_onlytumor_untilCluster1_alternativeclustering.RDATA")
load("Hwang_AUCell_onlytumor_untilCluster10.RDATA")
```

```{r}
# Explore marker gene filtering
...
Now For the enrichment analysis, hypergeometric test comparing the marker genes with the array of all the possible genes is performed
...
As before also check teh Enrichment analysis
```{r}
# Visualize the scores
sign_markers %>% ggplot(aes(x = avg_log2FC)) + geom_density() + geom_rug() + theme_classic()
...
### Cluster Marker genes Upsetplot
Now that both the markers of high and low have been computed, explore the overlap of each with the AUCell signatures and with each other 
```{r}
...
How does the ISRact scores relate to known metadata of Hwang et al? - patient distribution - treatment - Responses
...
ggplot(isrvsmetadata, aes(x = get(feature), y=ISR_ratio)) +
  ...
  ylab("ISR_ratio")
```
## Relation of ISRact clusters and signatures with Chemotherapy and response
...
```{r}
treatment = "CRTx" 
comparison = "rest" # "Untreated" | "rest"
# ISRact in comparison vs treatment (CAREFUL DIFFERENT SIZE)
## processing
if (comparison == "Untreated"){
...
} else if (comparison == "rest"){
  filt_data <- dplyr::mutate(isrvsmetadata, 
                            treatment_status =  if_else(treatment_status == treatment, treatment, "Others"),
                            treatment_status = fct(treatment_status, levels = c("Others", treatment)))
}

## statistical test
leveneTest(ISR_ratio ~ treatment_status, filt_data) #p=5.371e-05 -> H0 is discarded, unequal variance, should do Welch
...
violin_treatment <- ggplot(filt_data, aes(x = treatment_status, y=ISR_ratio, fill = treatment_status))+
  geom_violin() +
  geom_hline(yintercept = 0, linetype = "dashed")+
...
plot_name <- paste0("results/Figures/violin_treatment_", treatment, "VS", comparison, ".svg")
print(paste0("plotting to: ", plot_name))
ggsave(plot_name,
       width = 4,
       height = 3)
```

Now the same plots but with relation to response
...
In these last chunks we have seen how ISRactPCAdiff high cells are specially abundant in these patients. But on the other hand, cluster 5 cells are the ones that show the ISR-like phenotype. How does cluster 5 relate to the CRTx BRCAness etc

```{r}
# UMAP to check distribution of CRTx treated patient vs the rest
## processing
filt_data <- dplyr::mutate(isrvsmetadata,
  treatment_status = if_else(treatment_status == treatment, treatment, "Others"),
  treatment_status = fct(treatment_status, levels = c("Others", treatment))
...
is_CRTx <- dplyr::rename(filt_data, is_CRTx = "treatment_status") %>%
  dplyr::select(CellID, is_CRTx) %>%
  column_to_rownames("CellID")

seurat_object <- AddMetaData(seurat_object, metadata = is_CRTx)

## plot
umap_crtx <- DimPlot_scCustom(seurat_object, group.by = "is_CRTx", reduction = "umap",colors_use = c("lightgrey", "black"))

# Barplot to assess distribution of CRTx vs Other in each cluster
bar_crtx <- ggplot(filt_data, aes(x = seurat_clusters, fill = treatment_status)) +
  geom_bar(position = "stack", stat = "count") +
  scale_fill_manual(values = c(CRTx = "black", Others = "lightgrey")) +
  theme_classic() +
  scale_x_discrete(guide = guide_axis(angle = 90)) +
  scale_y_log10() +
  ylab("log10(nCells)")

(plot2) / (umap_crtx) / (bar_crtx) + plot_layout(heights = c(2, 2, 1), guides = "collect")
```

## Other analysis performed in this data 
