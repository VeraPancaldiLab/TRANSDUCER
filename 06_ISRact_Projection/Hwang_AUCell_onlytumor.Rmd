...
```{r}
FeaturePlot(seurat_object, reduction = "umap", features = AUC_names)
FeaturePlot(seurat_object, reduction = "umap", features = c("Moffitt_basal", "Moffitt_classical", "Bailey_squamous", "Bailey_progenitor","Collison_QM", "Collison_classical", "MALIGNANT CELLS", "GATA6", "cnv_score", "ISR_ratio")) 
```
...
plotGATA6 <-  FeaturePlot(seurat_object, reduction = "umap",  features = "GATA6", ncol = 2) & 
  scale_colour_gradient2(low = 'blue', mid = 'white', high = 'red') # change reductuion to "UMAP" | "umap" 
...
(plot1 | plot2 & NoLegend()) / (plot3 | plotGATA6) + plot_layout(heights = c(1,1), guides = 'collect')
```
...
neg_markers <- dplyr::filter(sign_markers, avg_log2FC < 0) %>% rownames()non
