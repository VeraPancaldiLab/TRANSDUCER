...
And the visual representation, whith an extra one for Basal/CLassical

```{r}
FeaturePlot(seurat_object, reduction = "umap", features = AUC_names)
FeaturePlot(seurat_object, reduction = "pca", features = c("Moffitt_basal", "Moffitt_classical", "Bailey_squamous", "Bailey_progenitor","Collison_QM", "Collison_classical", "MALIGNANT CELLS", "GATA6", "cnv_score", "ISR_ratio")) 
```
...
clusterISRactlow_neg <- rownames(dplyr::filter(sign_markers, avg_log2FC < 0)) %>% GeneSet(., setName=paste0("clusterISRactlow_neg(", length(.),")"))

geneSets_vs_markers <- GeneSetCollection(c(ISRactGeneSets,  clusterISRactlow_pos, clusterISRactlow_neg))
...
#### plots
  pos_enrich_H@result %>%  
    plot_bar_enrich(0.05,paste0("Hallmark-", ont," Upregulated in cluster", cluster_to_explore)) %>% print()
  
  neg_enrich_H@result %>%  
    plot_bar_enrich(0.05,paste0("Hallmark-", ont," Downregulated in cluster", cluster_to_explore)) %>% print()
...
```{r}
# Top 10 plot
sign_markers <- dplyr::filter(markers, p_val_adj < 0.01) %>% dplyr::arrange(p_val_adj)
...
clusterISRacthigh_neg <- rownames(dplyr::filter(sign_markers, avg_log2FC < 0)) %>% GeneSet(., setName=paste0("clusterISRacthigh_neg(", length(.),")"))

geneSets_vs_markers <- GeneSetCollection(c(ISRactGeneSets,  clusterISRacthigh_pos, clusterISRacthigh_neg,  clusterISRactlow_pos, clusterISRactlow_neg))
upset(fromList(geneIds(geneSets_vs_markers)), nsets = 6)
```
