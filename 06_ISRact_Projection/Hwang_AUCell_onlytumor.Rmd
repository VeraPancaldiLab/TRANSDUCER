...
plot5 <- VlnPlot(seurat_object, features = c("GATA6"), group.by = "seurat_clusters", pt.size = 0) + geom_hline(yintercept=0, linetype = "dashed")
...
markers <- FindMarkers(seurat_object, ident.1 = cluster_to_explore, group.by = 'seurat_clusters')saveeeeee
```

```{r}
# Explore marker pct filtering
 dplyr::filter(markers, p_val_adj < 0.01) %>% ggplot(aes(x=pct.1, y=pct.2, colour=avg_log2FC))+
  geom_point() + 
