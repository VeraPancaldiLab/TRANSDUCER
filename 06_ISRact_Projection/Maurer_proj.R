...
ensembl75 <- useEnsembl(biomart = "genes",
                        ...
                        version = 109,
                        host="https://grch37.ensembl.org")#listAttributes(ensembl75, page="feature_page")

annot_ensembl75 <- getBM(attributes = c('ensembl_gene_id',
                                        'hgnc_symbol'), mart = ensembl75)
...
projection_maurer <- predict(pca_pdx, column_to_rownames(maurer_normcount,"sample")) %>% 
  as_tibble(rownames = "sample") %>%
...
show_projection <- bind_rows(as_tibble(pca_full_df) %>% mutate(dataset = "Sauyeun PDX"),
                             as_tibble(projection_maurer %>% mutate(ISRact =  "Unknown",
                                                                  dataset = "Maurer")))

dplyr::mutate(show_projection, ISRact = str_replace(ISRact, 'ICA3', 'ISRact')) %>%
  dplyr::filter(dataset %in% c("Sauyeun PDX","PACAOMICS PDX", "CPTAC", "CCLE", "Maurer"), # "Sauyeun PDX","PACAOMICS PDX", "CPTAC", "CCLE"
...
## PAMG vs PC1
correlation_plotter(data = projection_maurer, col1 = "PAMG", col2 = "PC1", data_name = "Maurer")
